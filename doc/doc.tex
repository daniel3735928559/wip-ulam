...
\icode{experiment17}, we mean the function \icode{experiment17} in the
\icode{experiment.py} file in that repository.  Sometimes we will
refer to code in other files as well from the same repository, and
will mention where the code in question is located in each case.
...
\item Second, beyond that, there is some reason for believing
  conjectures of this form, which generally takes the form: ``There
  are many positive-density sets of the right sort (1-additive,
  sum-free, etc.), and the procedures for constructing the sum-free
  and Ulam sets that we are looking at are in some way `as greedy as
  possible', so what other sets could we get from those constructions
  if not some version of the positive-density ones we can construct?''
  We will suggest some possible more precise versions of this in what
  follows.
\item Finally, while literal positive-density makes the arguments
  reasonably straightforward, many of the arguments could possibly go
  through without it (mostly by changing ``large'' from meaning
  ``large relative to $N$'' to ``large relative to $|A_N|$''), as we
  shall discuss later.
\end{enumerate}

\subsubsection{Sum-free sets}
...
\subsubsection{Ulam sequences}

As we have said, it appears that the Ulam sequence has positive
(upper) density around $0.07$.  This might suggest for us the
conjecture:
...
\subsubsection{1-additive sequences}
...
sequences.  Recall also that for sum-free sets, (where we similarly
had many regular sum-free sets, such as the odd numbers), we could
easily generate examples of irregular sum-free sets as well, such as
the $A_\lambda$ from earlier (for any irrational $\lambda$), where
recall $A_\lambda$ was the set of integers that, when reduced (in
$\R$) modulo $\lambda$, land in the interval
$(\frac{\lambda}{3}, \frac{2\lambda}{3})$.

One might wonder whether we can similarly generate irregular
1-additive sets (such as $U(1,2)$ appears to be).  It turns out that
one can do this using the basic idea behind the $A_\lambda$
construction, but being more careful about it:
...
We note, however, that this construction necessarily gives a
1-additive set with a base of size 3, whereas the Ulam numbers have a
base of size 2.  So we might wonder whether there is a similar (if
more complicated) construction of this sort.  
...
\begin{proof}[Idea]
  ...
  $\lambda$ and $a$ a ``left bat'' in the range
  $(\frac{\lambda}{6}, \frac{\lambda}{3})$.  But now we will start
  with $c$ being the ``ball'' inside the ``table''
...
\subsubsection{Non-uniformity/Regularity}
