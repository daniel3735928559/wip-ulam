...
then by theorems of Finch [PAPER] and Schmerl and Speigel [PAPER], we
...
Finch's ``Patterns in 1-additive sequences'' [PAPER].  That said, many
...
interesting features.  In [PAPER], Stefan Steinerberger did exactly
...
where $e(x) = e_N(x) = e^{2\pi i x / N}$ (where often, $N$ will be
ommitted from the notation and understood from context).  Thus
$\widehat{f}$ is a function defined on all of $\R/\Z$.  

\end{definition}

This definition satisfies many properties, which are standard from
Fourier analysis and additive combinatorics [TAO/VU]:
...
as usual, $A_N = A \cap [N]$.  Then in the case of the Ulam sequence,
what is observed numerically is that for one particular value of
$\alpha \in \R/\Z$ (namely $\alpha = \ldots)$, that
$\widehat{1_A}(\alpha) \approx 0.8$, and for $k \in \Z$,
...
\item As noted in [STEINERBERGER] it looks like as we increase $N$
...
\item It looks at a glance like this distribution is supported on the
  ...
  literaly the case, but in [GIBBS] there is a conjecture in this
  direction.
...
regularity, as well one example of a proof of regularity, are: 

[PAPER] Finch's criterion for regularity

[PAPER] Schmerl and Speigel prove of regularity for $U(2, 2n+1)$
...
\[x_n = \sum_i 1(x_{n-e_i})\]

where $1(x)$ is 0 unless $x = 1$, in which case $1(x) = 1$.  In particular,
$0 < x_n \leq s$ for all odd $n > e_s$.  Note also that $x_n$ depends on a
...
simply because for any odd number $r \in S$, then since 2 in $S$ also,
the only way $r+2$ is not in $S$ is if $r+2 = a+b$ for some other
$a,b \in S$.  But $r+2$ is odd, so one of its summands has to be even.
The only other choice is $2+2v$, so this tells us
$r+2-(2+2v) = r - 2v$ is in $S$.
...
\begin{example}
For example, the odd positive integers are sum-free.  More generally,
if $A \subset \Z/m$ is sum-free, then the set of integers $x$ that
reduce to an element of $A$ modulo $m$ is also sum-free.  In fact,
even more generally, for any surjective homomorphism
$\pi : \Z \to \R/\Z$, if $A$ is a sum-free subset of $\R/\Z$, then
$\pi\inv(A)$ is a sum-free set of integers.  Clearly, any subset of a
sum-free set is sum-free also.  For one final and very different sort
of example, the set $\{x^n : x \in \Z, x > 1\}$ (for any fixed
$n > 2$) is also sum-free by Fermat's last theorem.
\end{example}
...
ultimately periodic [PAPER].  The converse is believed to be false,
with one of the simplest apparent counterexamples being
$\theta(\overline{01001})$ ($\overline{01001}$ meaning the binary
sequence that repeats the pattern $01001$ forever).  This is a set
$\{2, \ldots\}$ that has been computed extensively and for which no
period has been identified.  Nevertheless, there is no known example
of an ultimately periodic decision sequence for which we can prove its
sum-free set is non-regular.

\subsubsection{Density and regularity}

Clearly the maximal density a sum-free set can have is $\frac12$,
which is realised by the example of the odd positive integers.  Luczak
has given a sort of converse to this example, proving in [PAPER] that
if a sum-free set contains an even number, then, remarkably, its
density is automatically bounded above by $\frac25$.  The proof is
short, but a little delicate, and we shall recall a version of it in
this section.

The basic idea of the proof is to find disjoint subsets of $[N]$ that
are the same size as $A$, or of a size related to $A$.  For example,
if $a \in A$ is any element, then because $A$ is sum-free, $A_N$ and
$A_N+a$ are disjoint in $[N+a]$, and thus
$2|A_N|/N <= 1 + \frac{a}{N}$, which is how we deduce our earlier
statement about $A$ having density bounded by $\frac12$.

The proof breaks up into two cases: Where $A$ has any run of
consecutive elements and where it does not.

In the case where $A$ has an even element but no two consecutive
elements.  Note that if $A$ is all even elemenets, then $\frac12 A$ is
...
at least one even element.  Then, let $t$ be the minimal odd
difference (which does exist using the odd and even elements, and is
not 1, since there are no consecutive elements).  Write $t = x-y$ for
$x, y \in A$.  

This means that if $a \in A$, then $a+t-2$ cannot be in $A$ (else
$t-2$ would be a smaller odd difference than the minimal odd
...
course, finding two disjoint subsets of even size $|A|$ is already
easy, however this lets us in fact find three: Since $t=x-y$, this
means $B+x-y$ and $A$ are disjoint, meaning $B+x$ and $A+y$ are
disjoint.  But both of these are contained in a set of the form $A+a$
for some $a \in A$, so they are both also disjoint from $A$.  Thus we
have $A$, $A+y$, and $B+x$ all disjoint.  If we truncate $A$ to $A_N$,
then these are all disjoint subsets of $[N+x]$, and so

\[2|A_N|+|B_N| \leq N+x\].

So if we can relate $|B|$ to $|A|$ (for the moment using the shorthand
$B = B_N$, $A = A_N$), then we are done.  But
$B, B+1, A\setminus B, A\setminus B + 1, A\setminus B + 2$ are all
disjoint in $[N+2]$, so $2|B| + 3|A| - 3|B| \leq N+2$, i.e.
...
In the case where $A$ has $d$ consecutive elements
$a, a+1, \ldots, a+d-1$, say, the argument is similar in flavour but
the technical details are all slightly different.  This time, we let
$t$ be any difference for which $t+1, \ldots, t+d$ are all not
differences.  (This does exist: $a, a+1 \in A$ means $1 \notin A$, so
let $t$ be the maximum of $1, \ldots, a-1$ that is in $A-A$.  Then
nothing from $t$ to $a-1$ is in $A-A$ (by definition), and nothing
from $a$ to $a+d-1$ is either (since these are all in $A$), so at
least $d$ elements (and possibly more) immediately after $t$ are not
in $A-A$).  Again, say $t = x-y$ for $x, y \in A$.  

Then now if $B$ is the set of elements for which $b+1, \ldots, b+d-1$
are all not in $A$, then certainly the sets $A, B+1, \ldots, B+d-1$,
are all disjoint.  In fact, we can get one more than this: We can
shift all these sets by $a$ and they are still disjoint: $A+a, B+a+j$
($j=1, \ldots, d-1$).  But now since the $a+j$ are all in $A$, these
sets are all themselves disjoint from $A$ (since they are subsets of
$A+z$ for various $z \in A$.  Thus, again truncating at $N$, we have
two sets of size $|A_N|$ and $d-1$ sets of size $|B_N|$ all disjoint
and inside $[N+a+d-1]$.  Thus: 

\[2|A_N|+(d-1)|B_N| \leq N+a+d-\]

So again, we need control over the size of $|B_N|$ in terms of $|A_N|$
and we will be done.  But this time, we note that if $z \in A$, $z+t$
could be in $A$, but then because of the definition of $t$, none of
$z+t+1, \ldots, z+t+(d-1)$ can be in $A$ (lest one of
$t+1, \ldots, t+(d-1)$ lie in $A-A$).  Thus $A+t$, insofar as it
intersects $A$, in fact only intesects $B$.  Put another way, $A+t$
and $A \setminus B$ are disjoint.  Again, this is only two sets, but
we can use the same trick as before to make it three: Since $t = x-y$,
we can equally say $A+x$ and $(A \setminus B) + y$ are disjoint, at
which point these are also disjoint from $A$ (again, being subsets of
$A+z$ for various $z \in A$).  So we have three disjoint subsets of
$[N+x]$, of sizes $|A_N|$, $|A_N|$, and $|A_N|-|B_N|$, meaning
$3|A_N| - |B_N| \leq N+x$ or:
...
For $d \geq 2$, this is again bounded by $\frac25$ in the limit, so
the claimed bound follows.

\subsubsection{Irregular sum-free sets}

A construction of Cameron and Erdos in [CAMERON ERDOS] supplies an
...
in $[\frac \alpha 3, \frac {2\alpha}{3}]$.  $A_\alpha$ is clearly
sum-free, since it is sum-free modulo $\alpha$, but they prove that
for irrational $\alpha$, $A_\alpha$ is also not regular.

\subsection{Roth's theorem}

Roth's theorem is about the number 3-term arithmetic progressions
$x, y, z$ in a set $A \subseteq \Z^+$.  Specifically, if $A$ has
positive density $\delta$, then there is always at least one such
arithmetic progression.  Equivalently, there is always at least one
solution to $x+z=2y$.  

In a sum-free set $A$, we are asserting that there are no solutions to
$x+z=y$ (swapping around variable names to highlight the similarity),
so if we have such a set that we believe has positive density, we
might wonder what the proof of Roth's theorem has to say about such an
$A$.  After all, in the case of the slightly different equation
$x+z=2y$ it says that the set $A$ cannot exist.

\subsubsection{Density increment proof}

First, we look back on how Roth's theorem is proven.  

We start by working mod some $N$.  If $A$ is a set of density $\delta$
in $\Z/N$, then the number $S$ of solutions to $x+z=2y$ is counted by 
...
\subsubsection{Abelian arithmetic regularity}
...
In [this paper](https://arxiv.org/pdf/1301.4579v2.pdf), Eberhard,
Green, and Manners prove that for every e > 0, there is a set A such
...
\subsubsection{Energy increment proof of Roth's theorem}

\subsubsection{Green's triangle removal theorem}

\subsubsection{Quantitative bounds in finite fields}
...
on length-3 arithmetic progression-free sets in $\FF_4^n$ and
subsequent work by others pushing it to $\FF_3^n$).  I will start by
