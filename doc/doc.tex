...
\begin{tabular}{ll}
\begin{tabular}{|l|l|}
\hline $\ulam{1}{2}$ & $[2; 2, 3, 1, 11, 1, 1, 4, 1, \ldots]$ \\\hline $i$ & $P_i/Q_i$\csvreader{datafiles/cf_u1_2.csv}{}{\\\csvcoli & \csvcolii}\\\hline
\end{tabular}
&
\begin{tabular}{|l|l|}
\hline $\ulam{1}{3}$ & $[2; 4, 1, 1, 2, 24, 1, 3, 10, 1, \ldots]$ \\\hline $i$ & $P_i/Q_i$ \csvreader{datafiles/cf_u1_3.csv}{}{\\\csvcoli & \csvcolii}\\\phantom{a}&\phantom{a}\\\hline
\end{tabular}
\end{tabular}
\begin{tabular}{ll}
\begin{tabular}{|l|l|}
\hline $\ulam{2}{3}$ & $[5; 2, 1, 1, 5, 3, 5, 4, 1, 1, 1, \ldots]$ \\\hline $i$ & $P_i/Q_i$ \csvreader{datafiles/cf_u2_3.csv}{}{\\\csvcoli & \csvcolii}\\\phantom{a}&\phantom{a}\\\phantom{a}&\phantom{a}\\\hline
\end{tabular}
&
\begin{tabular}{|l|l|}
\hline $\theta(01001)$ & $[2; 1, 1, 53, 2, 1, 1, 5, 1, 17, 2, \ldots]$ \\\hline $i$ & $P_i/Q_i$ \csvreader{datafiles/cf_01001.csv}{}{\\\csvcoli & \csvcolii}\\\hline
\end{tabular}

\end{tabular}
\begin{tabular}{ll}
\begin{tabular}{|l|l|}
\hline $\theta(01010)$ & $[3; 2, 18, 1, 8, 1, 2, 6, 4, 12, 1, 1, \ldots]$ \\\hline $i$ & $P_i/Q_i$ \csvreader{datafiles/cf_01010.csv}{}{\\\csvcoli & \csvcolii}\\\phantom{a}&\phantom{a}\\\hline
\end{tabular}
&
\begin{tabular}{|l|l|}
\hline $\theta(10010)$ & $[3; 4, 1, 2, 2, 3, 5, 5, 3, 1, 21, 7, \ldots]$ \\\hline $i$ & $P_i/Q_i$ \csvreader{datafiles/cf_10010.csv}{}{\\\csvcoli & \csvcolii}\\\hline
\end{tabular}
\end{tabular}
...
between 0 and $\pi$.  We want to look at the values that are larger
than $\sqrt{N}$.  However, if we are looking at finite $N$, then while
this is the average size we still expect some random variation to put
some values over this mark.  To account for this, then, we look at
values above $\sqrt{2N}$ in the first 5000 values of each sequence:

\begin{table}
\caption{Spectrum of $\ulam{1}{2}$}
\begin{center}
\begin{tabular}{|lll|}
\hline
...
$\ulam{2}{3}$ or the various $\theta(s)$: 


\begin{table}
\caption{Spectrum of $\ulam{1}{3}$}
\begin{center}
\begin{tabular}{|lll|}
\hline
  $x$ with $N\widehat{A_N} > \sqrt{2N}$ & $N \widehat{A_N}(x)$ & $k\alpha \approx \pm x \mod{2\pi}$ 
...
\begin{table}
\caption{Spectrum of $\theta(01010)$}
\begin{center}
\begin{tabular}{|lll|}
\hline
  $x$ with $N\widehat{A_N} > \sqrt{2N}$ & $N \widehat{A_N}(x)$ & $k\alpha \approx \pm x \mod{2\pi}$ 
...
\begin{table}
\caption{Spectrum of $\theta(10010)$}
\begin{center}
\begin{tabular}{|lll|}
\hline
  $x$ with $N\widehat{A_N} > \sqrt{2N}$ & $N \widehat{A_N}(x)$ & $k\alpha \approx \pm x \mod{2\pi}$ 
...
Secondly, Plancherel tells us that over $x \in \frac1{N}\Z/\Z$, the
average value of

\[\left|\sum_{t=0}^{N-1} f(t) e(-2\pi xt)\right|\]

will be around $\sqrt{N}$, so the fact that in this case some entry 
...
$\lambda$, as we do in \icode{experiment10}, we get:
...
Which share many features with the distributions of the Ulam numbers
modulo $\lambda$, bolstering the idea that it isn't the feature of
``being an Ulam number'' as such that creates the distribution, but
more just the relationship between $r_{A+A}(x)$ and being in $A$.
Specifically, for \relevant sets $A$, $x \in A \iff r_{A+A}(x)$ is
small, whether it is 2 or 3 and so $x \in A$ or it is literally 0, and
$x$ is in the sets we're counting here, such $x$'s still show the same
bias modulo $\lambda$, 
...
computed by \icode{experiment12}: 
