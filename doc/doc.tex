...
and 4, and computational evidence in favour of the others.  Before 
...
\begin{enumerate}
\item We study the large Fourier coefficient $\alpha$ and
  ...
  \item We use a computer program to compute continued fraction
    ...
    least algebraic.  
  \item We study the constraints that would be imposed on a
    $\ulam{a}{b}$ if the corresponding $\lambda$ were to be rational
    with numerator 3. 
...
\item We then study the complete spectrum of \relevant sets $A$,
  computing via computer program 

Prove the existence of a large Fourier coefficient at
    some $\alpha$.
  \item Prove that the spectrum of $A$ is supported in $\alpha \Z$.  
  \item Prove that the Fourier coefficients $\widehat{A}(k\alpha)$
    decay fast enough as $k \to \infty$.  
...
\subsection{Structure of $\ulam{1}{2}$}
...
\subsubsection{Variants of the Ulam problem}
