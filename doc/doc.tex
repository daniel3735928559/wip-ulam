...
that we have been considering is that $x$ being in one of these sets
$A$ is determined by how many ways we can write $x$ as a sum $x = a+b$
for $a, b \in A$.  To study this, we make the following definitions:
...
being in $A$.  Specifically, for Ulam sequences $A$,
$x \in A \iff r_{A+A}(x)$ is small, whether it is 2 or 3 and so
...
\icode{experiment13}.  Some post-processing of the output gives us
