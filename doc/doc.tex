...
Considering this is step 1 in our strategy, we might wonder whether
what reasons there are for believing this.  After all, the only reason
we know other Ulam sequences such as $\ulam{2}{5}$ have positive
density is that we know they are regular.  So in trying to get a
handle on the distribution of $\ulam{1}{2}$, it might be backwards to
start with an assertion of positive density and then deduce
regularity.

Even so, it is already non-obvious that positive density implies all
the rest of what we want to know, and so if this is all we can prove
at the moment, it still seems worthwhile to record.  

But beyond that, we will give some reason to believe this conjecture.
