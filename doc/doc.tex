...
The statement that the set if chosen in this way should be thought of
as analogous to having only one large Fourier coefficient: Namely
$\widehat{1_{A_N}}(0) = pN$, with all the others being small.  So we
would hope to deduce more generally that this situation always implies
we have many solutions to $x+y=z$, giving that all the sets we have
been looking at cannot have only small Fourier coefficients.  Put
another way ``if a set is dense, then to avoid solutions to $x+y=z$ it
needs to have some kind of pattern (such as being all odds).''


\begin{theorem}\label{thm:alpha}
  If $A \subseteq \N$ is a sequence of positive integers of density
  $\delta$ such that $T(A_N)$ is bounded by $c N^{2-\epsilon}$ for
...
\begin{theorem}\label{thm:hole1}
  If $A$ is a set for which $r_{A+A}(x) > c$ implies $x \notin A$ for
  some constant $c$, and further, $A$ has spectrum $\alpha\Z$, then
...
\begin{proof}
  ...
  later.  let $|\widehat{1_A}(\alpha)| = \rho N$.  Finally, let $t_0$
  be such that 

  \[\sum_{t=t_0}^\infty \frac{1}{t^2} \leq \frac{\delta^2 + \rho^2 + \epsilon}{C^2}\]

  for some $\epsilon > 0$.  Then if we guarantee that
  $\normrz{t_0 x/\lambda} < \frac14$ (so that $\Re(e(tx)) > 0$ for all
  $t < t_0$), then for $N$ large enough, we have:
{\color{red}
  \begin{eqnarray*}
    r_{A_N - A_N}(x) &=& \frac{1}{N} \sum_{t=0}^{N-1} |\widehat{1_A}(t)|^2 e_N(tx)\\
                     &=& \delta^2 N + \frac{1}{N} |\widehat{1_A}(t)|^2 \Re(e_N(kx)) + \frac{1}{N} \sum_{t\neq 0,\pm k} |\widehat{1_A}(t)|^2 e_N(tx)\\
                     &\geq& \delta^2 N + \frac{2}{N} |\widehat{1_A}(t)|^2 \Re(e_N(\alpha x)) -
                            \frac{1}{N} \sum_{t=1}^\infty |\widehat{1_A}(t \alpha)|^2\\
                     &\geq& \delta^2 N + \rho^2 N -
                            N C^2 \sum_{t=2}^\infty \frac{1}{t^2}\\
                     &=& N\left(\delta^2 + \rho^2 - C^2 \sum_{t=t_0}^\infty \frac{1}{t^2}\right)\\
                     &\geq& \epsilon N\\
  \end{eqnarray*}
  
  (Using theorem \ref{thm:decay}, to conclude
  $|\widehat{1_A}(t\alpha)| \leq \frac{C}{t}$.)

  Thus for $x$ large enough and $N$ large enough, we get $O(N)$
  representations of $x$ in $A+A$.  
...
\[N \int_{\frac{t_0}{N}}^{2\pi} \frac{e^{it}}{t^2}\]
...
in terms of $t_0$ and $N$ and get a better result.
