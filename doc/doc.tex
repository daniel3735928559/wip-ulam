...
$\Omega(N^2)$ such sums.  If we have fewer of these, say
...
\item (Cross-correlation formula): For two functions $f, g : \Z/N \to \C$,
  we have $(\dft_N (f\overline{g}))(x) = (\dft_N f \star \dft_N g)(x)$ for all
  $x \in \Z/N$, where $(F \star G)(x) = \sum_{t=0}^{N-1} F(t)G(t+x)$ is
